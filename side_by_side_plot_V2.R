pfdata <- read.table(
  'http://www.stat.umn.edu/~corbett/classes/5303/RDataFiles/exmpl8.8', header=T)
...
side_by_side <- function(model, boxplot_threshold = 20, exclude_term = NULL)
{
  ...
  for (i in 1:length(term_labels))
  {
    ...
    # model.effects function processes a model object from lm or lme
    # it seems like model from aov is not processed properly...
...
# A test of the function with the data from exmpl 8.10
